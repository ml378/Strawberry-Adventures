...
enum AIType { WALKER, WALKER_BOSS, GUARD, FLYER, PROJECTILE };
...
public:
    ...
    float m_w_direction_timer = 0.0f;
    float m_w_direction_interval = 2.0f;
    int m_w_direction = -1;
    // Flyer
    float m_fly_timer = 0.0f;
    float m_direction_timer = 0.0f;
    float m_direction_interval = 2.0f;
    int m_direction = -1; 
    // Projectile
    float m_projectile_timer = 0.0f;
    float m_motion_timer = 0.0f;
    float m_motion_interval = 0.0f;
    ...
    void update(float delta_time, Entity* player, Entity* collidable_entities, int collidable_entity_count, Map* map, Map* grass);
    void render(ShaderProgram* program);

    void ai_activate(Entity* player, float delta_time);
    void ai_walk();
    void ai_guard(Entity* player);
    void ai_walker(float delta_time);
    void ai_walker_boss(Entity* player, float delta_time);
    void ai_flyer(Entity* player, float delta_time);
    void ai_projectile(float delta_time);
    ...
    void const set_start_position(glm::vec3 start_pos) { m_start_pos = start_pos; }
