...
void ShaderProgram::load(const char *vertex_shader_file, const char *fragment_shader_file) {
    ...
    m_loss_effect_uniform       = glGetUniformLocation(m_program_id, "life_loss");
...
void ShaderProgram::set_loss_effect(bool life_loss) {
    glUseProgram(m_program_id);
    glUniform1i(m_loss_effect_uniform, life_loss ? 1 : 0);
}
